<?php
    ...
    <urlset xmlns="https://www.sitemaps.org/schemas/sitemap/0.9">';
     ...
     $sql = "SELECT DISTINCT bname, state FROM banks LIMIT 0,50000";
     $result = $conn->query($sql);
    while($row = $result->fetch_assoc())
    { 
        
        $sitemap_url = ("https://ifsc.sarkaribank.com/".str_replace(' ', '-',strtolower($row['bname']))."/".str_replace(' ', '-',strtolower($row['state']))."/");
